...
(() => {
  ...
  const gradient = () => {
    ...
    if (document.body.classList.contains('sun-mode')) {
      return root.getPropertyValue('--ever-gold-gradient').trim() || 'linear-gradient(135deg,#fff0ae 0%,#d7a53a 52%,#9b5e08 100%)';
    }
    if (document.body.classList.contains('mono-mode')) {
      return 'linear-gradient(135deg,#fff 0%,#aaa 52%,#666 100%)';
    }
    return root.getPropertyValue('--ever-gradient').trim() || 'linear-gradient(135deg,#c9e2f5 0%,#8badcc 48%,#67e7dd 100%)';
  };
  ...
  function clearInline(el) {
    if (!el) return;
    for (const prop of ['background','background-image','background-color','color','border-color','box-shadow','filter','opacity']) {
  ...
  function sync(view = 'hub') {
    ...
    if (cat) {
      paint(cat, activeView === 'category' && cat.getAttribute('aria-expanded') === 'true');
    }
  ...
  function closeAllViews() {
    try { window.closeMessages?.(); } catch {}
    try { window.closeProfileView?.(); } catch {}
  ...
  function openMessagesView() {
    ...
    if (view && feed && window.matchMedia('(min-width: 961px)').matches && view.parentElement !== feed) {
