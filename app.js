...
if (profileLoginForm) {
  profileLoginForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      await Promise.all([loadPosts(), loadOnline()]);
    } catch (error) {
...
/* =========================================================
   FINAL NAVIGATION — HISTORY + MOBILE MENU ISLAND
   ========================================================= */
(() => {
  const railItems = document.querySelectorAll('.desktop-rail-action[data-rail-view]');

  const setRailActive = (view) => {
    railItems.forEach((item) => {
      item.classList.toggle('active', item.dataset.railView === view);
    });
  };

  const historyItem = document.querySelector('.desktop-rail-action[data-rail-view="history"]');
  ...
  const dock = document.getElementById('mobileDock');
  const menuButton = document.getElementById('mobileMenuNav');
  const menuPanel = document.getElementById('mobileMenuPanel');
  const menuContent = document.getElementById('mobileMenuPanelContent');
  const closeButton = document.getElementById('closeMobileMenu');
  const rightRail = document.querySelector('.right-rail');

  if (!dock || !menuButton || !menuPanel || !menuContent) return;

  let previousView = 'hub';

  const rememberCurrentView = () => {
    const active = dock.querySelector('.mobile-dock-item.active');
    if (active && active.dataset.view !== 'menu') {
      previousView = active.dataset.view;
    }
  };
  ...
  const fillMenu = () => {
    ...
    menuContent.replaceChildren(...Array.from(clone.children));
  };
  ...
  const openMenu = (event) => {
    ...
    dock.classList.add('menu-active');
    dock.classList.add('has-notifications');
  ...
  const closeMenu = (event) => {
    ...
    dock.classList.remove('menu-active');
    setMobileDockActive(previousView || 'hub');
    document.body.classList.remove('mobile-menu-open');
  };

  menuButton.addEventListener('click', openMenu, { capture: true });
  ...
  /* Do not show a notification badge until there is actually a notification. */
  dock.classList.remove('has-notifications');

  /* Restore a clean four-cell state after resize. */
  window.addEventListener('resize', () => {
    if (window.innerWidth > 660) {
      closeMenu();
      dock.classList.remove('has-notifications', 'menu-active');
    }
  });
})();
