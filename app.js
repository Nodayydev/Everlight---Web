...
function renderPost(post) {
  ...
  return `
    <article
      ...
      data-share-body="${encodeURIComponent(rawBody)}"
      tabindex="0"
...
function getPostShareData(card) {
  ...
  let rawShareBody = "";
  try {
    rawShareBody = card?.dataset?.shareBody ? decodeURIComponent(card.dataset.shareBody) : "";
  } catch (_) { rawShareBody = ""; }
  const fullEl = card?.querySelector("[data-full-body]");
...
async function createPostShareFile(data) {
  ...
  // Universal body lines: prefer original post text (\n = line, \n\n = stanza gap).
  const formattedBodyLines = (function () {
    const maxChars = 38;
    const maxLines = 20;
    const raw = String(data.rawBody || data.body || "").replace(/\r\n/g, "\n");
    ...
    const pushWrapped = (lines, segment) => {
      const styled = detectStyle(segment);
      if (!styled.text) return;
      const words = styled.text.split(" ");
      let cur = "";
    ...
    if (html && (!work || (work.split("\n").length < 2 && /<br|\/p>|\/div>/i.test(html)))) {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        pushWrapped(lines, part);
      }
    } else if (html) {
      // Fallback: DOM walk from rendered HTML
      const tmp = document.createElement("div");
      tmp.innerHTML = html;
      const paragraphs = [];
      let buf = [];
      ...
      for (const parts of paragraphs) {
        const onlyWs = !parts.length || parts.every((p) => !String(p.text || "").replace(/\s+/g, "").trim());
        if (onlyWs) {
          if (lines.length && !lines[lines.length - 1].blank) {
            lines.push({ text: "", blank: true, bold: false, italic: false, quote: false });
          }
        ...
        const style = parts[0] || {};
        pushWrapped(lines, joined);
        // apply dominant style from first run
        if (lines.length) {
          const last = lines[lines.length - 1];
          last.bold = !!style.bold;
          last.italic = !!style.italic;
          last.quote = !!style.quote;
        }
      }
    }
    ...
    if (lines.length > maxLines && limited.length && limited[limited.length - 1].text) {
      limited[limited.length - 1].text += "…";
    }
  ...
  for (const line of formattedBodyLines) {
    ...
    else estBodyH += bodyLineHeight;
  }
  const bodyEnd = bodyY + Math.max(bodyLineHeight, estBodyH);
  ...
  const bodySvg = formattedBodyLines.map((line) => {
    if (line.blank || !String(line.text || "").trim()) {
      bodyCursorY += Math.round(bodyLineHeight * 0.85);
      return "";
    }
    bodyCursorY += bodyLineHeight;
    const y = cardY + bodyY + bodyCursorY;
  ...
  const isSun = document.body.classList.contains("sun-mode") || localStorage.getItem("everlight-accent") === "sun";
  const accentColor = isSun ? "#e8c56a" : "#8badcc";
  const domainPrimary = "everlight.nodayy.hu";
  const domainSecondary = "nodayy.hu";
...
async function loadMessageContacts() {
  ...
  try {
    ...
    const upsert = (u) => {
      ...
      if (uname.startsWith("anonymous") || uname === "névtelen") return;
    ...
    contactsList.innerHTML = merged.map((user) => {
      ...
      const cache = window.__streakByUser || {};
      const bare = String(user.username || "").split("#")[0].toLowerCase();
      const streak = Math.max(
        Number(user.message_streak || 0),
        Number(cache["id:" + user.id] || 0),
        Number(cache["u:" + String(user.username || "").toLowerCase()] || 0),
        Number(cache["b:" + bare] || 0)
      );
      return `
...
async function loadDailyThoughts() {
  ...
  // Cache streaks for the contacts list
  window.__streakByUser = window.__streakByUser || {};
  for (const user of users) {
    if (!user) continue;
    const s = Number(user.message_streak || user.streak || 0);
    if (user.id != null) window.__streakByUser["id:" + user.id] = Math.max(window.__streakByUser["id:" + user.id] || 0, s);
    if (user.username) {
      window.__streakByUser["u:" + String(user.username).toLowerCase()] = Math.max(window.__streakByUser["u:" + String(user.username).toLowerCase()] || 0, s);
      const bare = String(user.username).split("#")[0].toLowerCase();
      window.__streakByUser["b:" + bare] = Math.max(window.__streakByUser["b:" + bare] || 0, s);
    }
  }
}
...
(function initMobileCommunitySheet(){
  ...
  const source = document.querySelector(".right-rail:not(.mobile-cloned-rail)");
