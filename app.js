...
function renderPost(post) {
  ...
  let title = markdownToPlain(lines[0] || category);
  let excerpt = lines.length > 1
    ? markdownToPlain(lines.slice(1).join(" "))
  ...
  if (lines.length === 1 && title.length > 68) {
    ...
    if (sentence) {
      ...
      excerpt = markdownToPlain(rawBody.slice(title.length).trim());
    } else {
      title =
        title.slice(0, 68).trimEnd() + "…";

      excerpt = markdownToPlain(rawBody);
    }
  }
