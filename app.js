...
function renderPost(post) {
  const date = post.created_at
    ? new Date(post.created_at).toLocaleTimeString("hu-HU", { hour: "2-digit", minute: "2-digit" })
    : "";
  ...
  const displayName = anonymous
    ? "Névtelen"
    : (post.display_name || (post.username || "Felhasználó").split("#")[0] || "Felhasználó");
  const username = anonymous
    ? "@névtelen"
    : (post.username ? `@${post.username}` : "");

  let rawBody = String(post.body || "").trim();

  /*
   * A régebbi editor-verziók néha ugyanazt a sort kétszer mentették: 
   * egyszer formázott címként, majd egyszer normál szövegként.
   * A feed nem használ külön címet, ezért az ilyen ismétlést itt
   * normalizáljuk, így a régi bejegyzések is csak egyszer jelennek meg.
   */
  const rawLines = rawBody.split(/\r?\n/).map(line => line.trim()).filter(Boolean);
  if (rawLines.length >= 2) {
    const firstPlain = markdownToPlain(rawLines[0]);
    const secondPlain = markdownToPlain(rawLines[1]);
    if (firstPlain && firstPlain === secondPlain) {
      rawBody = rawLines.slice(0, 1).join("\n");
    }
  }

  const plainBody = markdownToPlain(rawBody);
  const isLongStory = category === "Történet" && plainBody.length > 420;
  const shouldCollapse = isLongStory || plainBody.length > 520;
  const previewLength = category === "Történet" ? 420 : 520;
  const preview = shouldCollapse
    ? plainBody.slice(0, previewLength).trimEnd() + "…"
    : plainBody;

  const avatar = post.avatar
    ? `<img class="feed-post-avatar-image" src="${escapeHtml(post.avatar)}" alt="" loading="lazy">`
    : `<span class="feed-post-avatar-fallback">${escapeHtml((displayName || "?").slice(0, 1).toUpperCase())}</span>`;

  const imageMarkup = post.image
    ? `<figure class="feed-post-image-wrap">
         <img class="feed-post-image" src="${escapeHtml(post.image)}" alt="Megosztott kép" loading="lazy">
       </figure>`
    : "";

  const bodyMarkup = shouldCollapse
    ? `<div class="feed-post-text is-collapsed" data-post-text>${escapeHtml(preview)}</div>
       <button class="feed-post-more" type="button" data-expand-post>Mutasd teljesen ↓</button>
       <div class="feed-post-full" data-full-post-text hidden>${escapeHtml(rawBody)}</div>`
    : `<div class="feed-post-text" data-post-text>${markdownToSafeHtml(rawBody)}</div>`;

  return `
    <article class="post feed-card feed-post" data-post-id="${escapeHtml(String(post.id || ""))}">
      <header class="feed-post-header">
        <div class="feed-post-avatar">${avatar}</div>
        <div class="feed-post-author">
          <strong>${escapeHtml(displayName)}</strong>
          <span>${escapeHtml(username)}${date ? ` · ${escapeHtml(date)}` : ""}</span>
        </div>
      </header>
      ...
      <div class="feed-post-main">
        ...
        <div class="feed-post-meta">
          <span class="feed-post-category">${escapeHtml(category)}</span>
        </div>
        <div class="feed-post-actions" aria-label="Bejegyzés műveletei">
          <button type="button" aria-label="Válasz">◯</button>
          <button type="button" aria-label="Megosztás">↗</button>
          <button type="button" aria-label="Kedvelés">♡</button>
          <button type="button" aria-label="Mentés">⌑</button>
          ${currentUser?.isOwner ? `<button type="button" class="feed-post-delete" data-delete-post aria-label="Bejegyzés törlése">⌫</button>` : ""}
        </div>
      </div>
    </article>
  `;
...
/* Hosszú történetek a feeden belül nyílnak ki; nincs külön ablak vagy oldal. */
document.addEventListener("click", (event) => {
  ...
  const card = expand.closest(".feed-post");
  if (!card) return;

  const preview = card.querySelector("[data-post-text]");
  const full = card.querySelector("[data-full-post-text]");
  if (!preview || !full) return;

  preview.innerHTML = markdownToSafeHtml(full.textContent || "");
    ...
    community.innerHTML = `
      <div class="community-latest-list">
        ${latestPosts.map((post) => {
          ...
          let title = markdownToPlain(lines[0] || post.category || "Bejegyzés");
...
(() => {
  ...
  const wrapSelection = (before, after = before) => {
    ...
    replaceSelection(replacement, before.length, before.length + selected.length);
  };
  ...
  const insertText = (text) => replaceSelection(text, text.length, text.length);
  ...
  toolbar.addEventListener("click", event => {
    ...
    if (action === "heading") return prefixLines("# ");
    if (action === "bullet") return prefixLines("• ");
    if (action === "quote") return prefixLines("> ");
    if (action === "link") return wrapSelection("[", "](https://)");
    if (action === "code") return wrapSelection("`", "`");
    if (action === "emoji") return insertText(" ✦");
    if (action === "table") return insertText("\n| Cím | Érték |\n| --- | --- |\n| Példa | Szöveg |\n");
    if (action === "image" || action === "media") return document.getElementById("imageInput")?.click();
    if (action === "undo") return undo();
    if (action === "redo") return redo();
    if (action === "save") {
      localStorage.setItem("everlight-feed-draft", editor.value);
      notify("A piszkozat elmentve ezen az eszközön.");
      return;
    }
    if (action === "preview") {
      const value = editor.value.trim();
      if (!value) return notify("Az előnézethez írj valamit.");
      let modal = document.getElementById("feedEditorPreview");
      if (!modal) {
        modal = document.createElement("div");
        modal.id = "feedEditorPreview";
        modal.className = "feed-editor-preview-modal";
        modal.innerHTML = `<div class="feed-editor-preview-backdrop" data-preview-close></div><section class="feed-editor-preview-card" role="dialog" aria-modal="true"><button type="button" class="feed-editor-preview-close" data-preview-close>×</button><div class="feed-editor-preview-body"></div></section>`;
        document.body.appendChild(modal);
        modal.addEventListener("click", e => { if (e.target.closest("[data-preview-close]")) modal.classList.remove("open"); });
      }
      modal.querySelector(".feed-editor-preview-body").innerHTML = markdownToSafeHtml(value);
    ...
    if (action === "format") return wrapSelection("~~", "~~");
    if (action === "paragraph") return prefixLines("");
  });
    ...
    async () => {
      ...
      try {
        ...
        if (feedList) {
          ...
          /* Az üres/állapotjelző sort ne hagyjuk a frissen létrehozott
             bejegyzés alatt. Ez okozta a „Még nincs bejegyzés” duplázást. */
          feedList.querySelectorAll(".loading-copy").forEach((node) => node.remove());
