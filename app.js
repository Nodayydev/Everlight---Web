...
function renderPost(post) {
  const date = new Date(post.created_at).toLocaleTimeString("hu-HU", {
    hour: "2-digit",
    minute: "2-digit"
  });
  ...
  const displayName = anonymous
    ? "Névtelen"
    : (post.display_name || post.username || "Felhasználó");
  ...
  // A törzs mindig csak egyszer jelenik meg. A cím kizárólag a külön címezhető mezőből jön.
  const excerptText = markdownToPlain(rawBody);
  const visibleExcerpt = excerptText.length > 320
    ? excerptText.slice(0, 320).trimEnd() + "…"
    : excerptText;
  ...
  const imageMarkup = post.image
    ? `
      <div class="feed-card-media">
        <img class="post-image" src="${escapeHtml(post.image)}" alt="Megosztott kép" loading="lazy">
      </div>
    `
    : "";
  ...
  const handleMarkup = handle
    ? `<span class="feed-card-handle">${escapeHtml(handle)}</span>`
    : "";
  ...
  return `
    <article
      ...
      tabindex="0"
      role="button"
      aria-label="${escapeHtml(postTitle || category)} – bejegyzés megnyitása"
    >
      ${imageMarkup}
      <div class="feed-card-content">
        <div class="feed-card-author">
          ...
          <time datetime="${escapeHtml(String(post.created_at || ""))}">${escapeHtml(date)}</time>
        </div>
        ...
        <p class="feed-card-excerpt">${escapeHtml(visibleExcerpt)}</p>
      </div>
      ...
      <footer class="feed-card-actions" aria-label="Bejegyzés műveletei">
        ...
        <button type="button" class="feed-action${post.saved ? " is-active" : ""}" data-post-action="save" data-post-id="${escapeHtml(String(post.id || ""))}" aria-label="Mentés könyvjelzőként" aria-pressed="${post.saved ? "true" : "false"}">
