...
if (profileLoginForm) {
  profileLoginForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const note = document.getElementById("profileLoginError");
      if (note) {
        note.textContent = error?.message || "A bejelentkezés nem sikerült.";
        note.hidden = false;
      }
    } finally {
      if (submitButton) {
  ...
  return `
    ...
    >
      ...
      <footer class="feed-card-actions" aria-label="Bejegyzés műveletei">
        ...
        ${currentUser && Number(post.author_id) === Number(currentUser.id) ? `
        <button type="button" class="feed-action feed-comments-toggle" data-comments-toggle data-post-id="${escapeHtml(String(post.id || ""))}" aria-label="Hozzászólások be- vagy kikapcsolása" aria-pressed="${post.comments_enabled === false || Number(post.comments_enabled) === 0 ? "false" : "true"}" title="Hozzászólások ${post.comments_enabled === false || Number(post.comments_enabled) === 0 ? "bekapcsolása" : "kikapcsolása"}">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5.5A2.5 2.5 0 0 1 6.5 3h11A2.5 2.5 0 0 1 20 5.5v7A2.5 2.5 0 0 1 17.5 15H11l-4.5 4v-4h0A2.5 2.5 0 0 1 4 12.5v-7Z" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"/><path d="M8 8h8M8 11h5" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/></svg>
      ...
      ${post.comments_enabled === false || Number(post.comments_enabled) === 0 ? `
      <section class="feed-card-comments feed-comments-disabled" data-comments-section data-post-id="${escapeHtml(String(post.id || ""))}">
        <div class="feed-comments-head">
          <strong>Hozzászólások</strong>
          <span>Kikapcsolva</span>
        </div>
      </section>` : `
      <section class="feed-card-comments" data-comments-section data-post-id="${escapeHtml(String(post.id || ""))}" data-comments-open="false">
        <div class="feed-comments-head">
          <strong>Hozzászólások</strong>
          <span data-comment-count>${escapeHtml(String(post.comment_count || 0))}</span>
          <button type="button" class="feed-comments-open-button" data-comments-open-toggle aria-expanded="false" aria-label="Hozzászólások megnyitása">Megnyitás</button>
        </div>
        <div class="feed-comments-body" data-comments-body hidden>
...
function refreshCommentSections() {
  // A hozzászólások alapból zárva vannak; csak megnyitáskor töltjük be őket.
}
...
document.addEventListener("keydown", (event) => {
  // A bejegyzés kártya billentyűzetes nyitása ne kapja el az
  // űrlapmezőkben használt Space/Enter billentyűket.
  // Így a hozzászólásokban a szóköz és az Enter normálisan működik.
...
async function createPostShareFile(data) {
  ...
  // Instagram/TikTok/Snapchat story-ready canvas: always 9:16.
  // The original Everlight post is placed inside a rounded card on a
  // branded background, rather than being exported as a plain cropped post.
  const targetWidth = 1080;
  const targetHeight = 1920;
  const side = 72;
  const cardX = 48;
  const cardW = targetWidth - 96;
  const cardY = 300;
  const cardBottom = 1635;
  ...
  const bodyLines = wrapShareText(data.body, 55, 9);
  const avatarSize = 88;
  ...
  const avatarMarkup = avatarData
    ? `<defs><clipPath id="storyAvatarClip"><circle cx="${avatarX + avatarSize/2}" cy="${avatarY + avatarSize/2}" r="${avatarSize/2}"/></clipPath></defs><image href="${escapeXml(avatarData)}" x="${avatarX}" y="${avatarY}" width="${avatarSize}" height="${avatarSize}" preserveAspectRatio="xMidYMid slice" clip-path="url(#storyAvatarClip)"/>`
    : `<circle cx="${avatarX + avatarSize/2}" cy="${avatarY + avatarSize/2}" r="${avatarSize/2}" fill="#18272b"/><text x="${avatarX + avatarSize/2}" y="${avatarY + 56}" text-anchor="middle" fill="#9fc4e7" font-size="38" font-family="Arial">✦</text>`;

  const meta = [data.realName, data.handle].filter(Boolean).join("  ");
  const titleStartY = cardY + 230;
  const bodyStartY = titleStartY + titleLines.length * titleLineHeight + 38;
  const bodySvg = bodyLines.map((line, i) =>
    `<text x="${cardX + 42}" y="${bodyStartY + i * bodyLineHeight}" fill="#d5dcde" font-size="${bodyFont}" font-family="Arial">${escapeXml(line)}</text>`
  ).join("");
  const titleSvg = titleLines.map((line, i) =>
    `<text x="${cardX + 42}" y="${titleStartY + i * titleLineHeight}" fill="#f4f6f6" font-size="${titleFont}" font-weight="800" font-family="Arial">${escapeXml(line)}</text>`
  ).join("");

  const dividerY = Math.min(cardBottom - 155, bodyStartY + Math.max(1, bodyLines.length) * bodyLineHeight + 36);
  const urlLabel = `${window.location.host} · Bejegyzés megnyitása`;
  const footerY = cardBottom - 54;
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${targetWidth}" height="${targetHeight}" viewBox="0 0 ${targetWidth} ${targetHeight}">
    <defs>
      <linearGradient id="storyBg" x1="0" y1="0" x2="1" y2="1">
        ...
        <stop offset="52%" stop-color="#111a1e"/>
        <stop offset="100%" stop-color="#050708"/>
      </linearGradient>
      <radialGradient id="storyGlow" cx="50%" cy="35%" r="70%">
        <stop offset="0%" stop-color="#8badcc" stop-opacity=".18"/>
        <stop offset="100%" stop-color="#8badcc" stop-opacity="0"/>
      </radialGradient>
      <filter id="storyShadow" x="-20%" y="-20%" width="140%" height="140%">
        <feDropShadow dx="0" dy="24" stdDeviation="28" flood-color="#000000" flood-opacity=".65"/>
      </filter>
    </defs>

    <rect width="1080" height="1920" fill="url(#storyBg)"/>
    <rect width="1080" height="1920" fill="url(#storyGlow)"/>
    <circle cx="80" cy="230" r="180" fill="#8badcc" opacity=".05"/>
    <circle cx="1030" cy="1560" r="260" fill="#55d7cf" opacity=".045"/>

    <text x="72" y="120" fill="#8badcc" font-size="25" font-weight="700" letter-spacing="6" font-family="Arial">EVERLIGHT.</text>
    <text x="72" y="158" fill="#6f7d82" font-size="18" letter-spacing="3" font-family="Arial">GONDOLATOK · TÖRTÉNETEK · EMBEREK</text>

    <g filter="url(#storyShadow)">
      <rect x="${cardX}" y="${cardY}" width="${cardW}" height="${cardH}" rx="46" fill="#080b0d" stroke="#2a3940" stroke-width="3"/>
      <rect x="${cardX}" y="${cardY}" width="${cardW}" height="185" rx="46" fill="#11191d"/>
      <rect x="${cardX}" y="${cardY + 145}" width="${cardW}" height="40" fill="#11191d"/>
      ${avatarMarkup}
      <text x="${textX}" y="${avatarY + 38}" fill="#f1f4f5" font-size="34" font-weight="800" font-family="Arial">${escapeXml(data.accountName)}</text>
      <text x="${textX}" y="${avatarY + 72}" fill="#849196" font-size="22" font-family="Arial">${escapeXml(meta)}</text>
      <text x="${cardX + cardW - 42}" y="${avatarY + 70}" text-anchor="end" fill="#7f8b90" font-size="20" font-family="Arial">${escapeXml(data.date)}</text>
      <line x1="${cardX + 42}" y1="${cardY + 185}" x2="${cardX + cardW - 42}" y2="${cardY + 185}" stroke="#273238" stroke-width="2"/>
      ${titleSvg}
      ${bodySvg}
      <line x1="${cardX + 42}" y1="${dividerY}" x2="${cardX + cardW - 42}" y2="${dividerY}" stroke="#263238" stroke-width="2"/>
      <text x="${cardX + 42}" y="${footerY}" fill="#8badcc" font-size="22" font-family="Arial">Everlight</text>
      <text x="${cardX + cardW - 42}" y="${footerY}" text-anchor="end" fill="#7f8b90" font-size="19" font-family="Arial">${escapeXml(urlLabel)}</text>
    </g>

    <text x="540" y="1750" text-anchor="middle" fill="#e8eeee" font-size="25" font-weight="700" font-family="Arial">Nyisd meg az eredeti bejegyzést az Everlighton</text>
    <rect x="260" y="1790" width="560" height="64" rx="32" fill="#8badcc"/>
    <text x="540" y="1832" text-anchor="middle" fill="#071015" font-size="23" font-weight="800" font-family="Arial">${escapeXml(data.url)}</text>
  </svg>`;
          ...
          if (destination === "story" || destination === "close-friends") {
            ...
            if (shareFile && navigator.canShare?.({ files: [shareFile] })) {
              await navigator.share({
                ...
                text: `Megosztás az Everlighton: ${data.url}`,
                files: [shareFile],
                url: data.url
              });
            } else {
...
document.addEventListener('click', (event) => {
  ...
  const open = section.dataset.commentsOpen === 'true';
  section.dataset.commentsOpen = open ? 'false' : 'true';
  body.hidden = open;
  button.setAttribute('aria-expanded', open ? 'false' : 'true');
  button.setAttribute('aria-label', open ? 'Hozzászólások megnyitása' : 'Hozzászólások bezárása');
  button.textContent = open ? 'Megnyitás' : 'Bezárás';
...
document.addEventListener('click', async (event) => {
  ...
  try {
    const data = await api(`/api/posts/${encodeURIComponent(postId)}/comments/toggle`, { method: 'POST' });
    const card = button.closest('.feed-card');
    if (card) {
      const section = card.querySelector('[data-comments-section]');
      if (section) {
        if (data.comments_enabled) {
          section.outerHTML = `<section class="feed-card-comments" data-comments-section data-post-id="${escapeHtml(String(postId))}" data-comments-open="false">
            <div class="feed-comments-head"><strong>Hozzászólások</strong><span data-comment-count>0</span><button type="button" class="feed-comments-open-button" data-comments-open-toggle aria-expanded="false" aria-label="Hozzászólások megnyitása">Megnyitás</button></div>
            <div class="feed-comments-body" data-comments-body hidden>
              <div class="feed-comments-list" data-comments-list><span class="feed-comments-loading">Hozzászólások betöltése…</span></div>
              <form class="feed-comment-form" data-comment-form data-post-id="${escapeHtml(String(postId))}"><input type="text" name="body" maxlength="1000" autocomplete="off" placeholder="Írj egy hozzászólást…" aria-label="Hozzászólás írása"><button type="submit" aria-label="Hozzászólás küldése">↑</button></form>
            </div>
          </section>`;
          const newSection = card.querySelector('[data-comments-section]');
          if (newSection) loadPostComments(newSection);
        } else {
          section.outerHTML = `<section class="feed-card-comments feed-comments-disabled" data-comments-section data-post-id="${escapeHtml(String(postId))}"><div class="feed-comments-head"><strong>Hozzászólások</strong><span>Kikapcsolva</span></div></section>`;
        }
      }
    }
    button.setAttribute('aria-pressed', data.comments_enabled ? 'true' : 'false');
    button.title = `Hozzászólások ${data.comments_enabled ? 'kikapcsolása' : 'bekapcsolása'}`;
  } catch (error) {
    notify(error.message || 'A hozzászólások állapotát nem sikerült módosítani.');
  } finally {
...
const forgotPasswordStatus = $("#forgotPasswordStatus");
...
function setPasswordResetStatus(message = "", type = "") {
  ...
  forgotPasswordStatus.classList.toggle("is-error", type === "error");
  forgotPasswordStatus.classList.toggle("is-success", type === "success");
}
...
function openPasswordResetModal(resetToken = "") {
  ...
  if (resetToken) {
    ...
    if (intro) intro.textContent = "Állíts be egy új jelszót. A link 30 percig érvényes.";
  } else {
    if (forgotPasswordForm) forgotPasswordForm.hidden = false;
    if (resetPasswordForm) resetPasswordForm.hidden = true;
    if (intro) intro.textContent = "Add meg azt az e-mail címet, amelyet a fiókodhoz megadtál. Csak e-maillel rendelkező fiókokhoz küldünk visszaállító linket.";
  }
}
...
forgotPasswordForm?.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    setPasswordResetStatus(data.message || "Ha jogosult, elküldtük a visszaállító e-mailt.", "success");
    forgotPasswordForm.reset();
  } catch (error) {
    setPasswordResetStatus(error?.message || "A kérés nem sikerült.", "error");
  } finally {
...
resetPasswordForm?.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    setTimeout(() => closePasswordResetModal(), 1400);
  } catch (error) {
    setPasswordResetStatus(error?.message || "A jelszó visszaállítása nem sikerült.", "error");
  } finally {
    if (submit) submit.disabled = false;
  }
});
