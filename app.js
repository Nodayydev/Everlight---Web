...
function renderPost(post) {
  ...
  const avatarMarkup = anonymous
    ...
    : (post.avatar
      ...
      : `<div class="feed-card-avatar" aria-hidden="true">${escapeHtml((accountName.charAt(0) || "?").toUpperCase())}</div>`);
...
function openMobilePostShareSheet(card) {
  ...
  sheet.addEventListener("click", async (event) => {
    ...
    // The selection sheet itself stays HTML. Only after the user chooses a
    // destination do we invoke the browser's native share mechanism.
    if (destination === "story" || destination === "close-friends" || destination === "message") {
      try {
        if (navigator.share) {
          await navigator.share({ title: data.title || "Everlight", text: data.text, url: data.url });
        } else {
          await copyPostShareLink(data);
        }
      } catch (error) {
...
newMessageButton?.addEventListener("click", openNewMessageComposer);
composeNewButton?.addEventListener("click", openNewMessageComposer);
